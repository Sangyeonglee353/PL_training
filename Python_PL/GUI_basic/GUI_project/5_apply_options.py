...
import tkinter.messagebox as msgbox # 메시지 박스를 위해 추가
from tkinter import *
from tkinter import filedialog # filedialog는 sub 모듈이므로 따로 import
...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
        ...
        initialdir=r"D:\programing\Study\Python_PL\GUI_basic") # r을 붙이면 해당 경로를 그대로 사용함, 
...
def del_file():
    ...
    for index in reversed(list_file.curselection()): # 거꾸로 목록 가져오기
        list_file.delete(index)

# 저장 경로 (폴더)
def browse_dest_path():
    folder_selected = filedialog.askdirectory() # intitaldir에서 설정한 경로를 가져옴
    if folder_selected == '': # 사용자가 취소할 경우
        return
    #print(folder_selected)
    txt_dest_path.delete(0, END) # Entry로 만들었기 때문에 이렇게 삭제 | 리스트 삭제: .delete("1.0", END)
...
def merge_image():
    ...
    try:
        ...
        y_offset = 0 # x좌표는 그대로 y 위치 정보만 변경
        # for img in images:
        #     result_img.paste(img, (0, y_offset))
        #     y_offset += img.size[1] # height 값 만큼 더해짐
        ...
        # 포맷 옵션 처리
        file_name = "I_can_photo." + img_format
...
txt_dest_path.pack(side="left", fill="x", expand=True, padx=5, pady=5, ipady=4) # 높이 변경 ipady
