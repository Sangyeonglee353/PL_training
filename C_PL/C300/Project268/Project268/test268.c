...
#define ADDRFILE "D:\\mysources\\addrlist.txt";
...
void get_addrlist(void)
{
	...
	while (!feof(fp))
	{
		...
		if (add_list(&addr) == 0)
		{
			printf("주소 데이터를 링크드 리스트에 추가할 수 업습니다. \n");
		}
	}
...
int add_list(const ADDR* addr)
{
	...
	//g_pAddrHead가 초기화 되지 않은 경우, 한 번만 실행됩니다.
	if (g_pAddrHead == NULL)
	{
	...
	else // g_AddrHead가 초기화 된 후 계속 실행됩니다.
	{
		plocal = malloc(sizeof(ADDR); // ADDR 구조체 할당
		memset(plocal, 0, sizeof(ADDR));
